...
def imagenet(data_path, transform, train=True):
  ...
  if train:
    ...
    inids = [int(ln.strip()) - 1 for ln in
             open(os.path.join(data_path, 'anno', 'ILSVRC2012_validation_ground_truth.txt'))]
...
def imagenet100(data_path, transform, train=True):
  ...
  in100_cls_list = ['n02869837', 'n01749939', 'n02488291', 'n02107142', 'n13037406', 'n02091831', 'n04517823',
...
class SubstationHDF5Dataset(data.Dataset):
  ...
  def __init__(
    ...
    print(f"Substation {split} set: {len(self.indices)} samples")
...
class PASTISDataset(data.Dataset):
  """
  PASTIS semantic segmentation dataset.
  Folds {1,2,3} = train, {4} = val, {5} = test.
  Uses the first timestep of the S2 time series and BGR+NIR bands ([0,1,2,6]).
  ...
  def __init__(
    ...
    self.h5_path = os.path.join(data_path, h5_file)
    self.split = split
    self.img_size = to_2tuple(img_size)
    self.normalize = normalize
    self.tiles_per_img = tiles_per_img
    ...
    self.norm_config = json.load(open(os.path.join(data_path, norm_stats_file), 'r'))
  ...
  def __getitem__(self, idx):
    ...
    image, mask = self._resize_img_and_mask(image, mask)  # TF.crop/resize broadcasts over T
